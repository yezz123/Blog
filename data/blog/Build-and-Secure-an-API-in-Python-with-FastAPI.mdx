...
- Now we Create all our routes we need to Secure them that why i use `python-multipart` to implement and use `OAuth2PasswordBearer` for Authentication.
...
- This file is relate to our `Token` cause when we `get_current_user` we eed to verify the `token` after login when we use `oauth2_scheme = OAuth2PasswordBearer(tokenUrl="login")`.
- `oa2.py` :
